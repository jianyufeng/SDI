...
public class AppCRC {
	 ...
	 * @return
	 */
	public static byte[] GetCRC(ByteBuf data) {

		int startIndex = 0;
		int usDataLen = data.readableBytes()-2;
		byte uchCRCHi = (byte) 0xFF; /* high byte of CRC initialized */
		byte uchCRCLo = (byte) 0xFF; /* low byte of CRC initialized */
		int i = startIndex;
		int uIndex;
		while (usDataLen-- != 0) {
			uIndex = (uchCRCHi ^ ((byte) data.getByte(i))) & 0xff;
			uchCRCHi = (byte) (uchCRCLo ^ auchCRCHi[uIndex]);
			uchCRCLo = auchCRCLo[uIndex];
			i++;
		}

		short val = (short) ((uchCRCLo & 0xFF) << 8 | (uchCRCHi & 0xFF));
		//int value = ((((int) uchCRCHi) << 8 | (((int) uchCRCLo) & 0xff))) & 0xffff;
		byte[] shortBuf = new byte[2];
		//System.out.println("crc++" + ByteUtil.putShort(shortBuf, val, 0));
//		System.out.println("crc++" + bytesToHexString(ByteUtil.putInt(shortBuf, value, 0)));

		shortBuf[1] = (byte) (val >> 8);
		shortBuf[0] = (byte) (val >> 0);
		return /*ByteUtil.putShort(shortBuf, val, 0)*/shortBuf;
	...
	/*public static byte[] GetCRC(byte[] data) {
		...
		int uIndex;
		while (usDataLen-- != 0) {
			uIndex = (uchCRCHi ^ ((byte) data[i])) & 0xff;
			uchCRCHi = (byte) (uchCRCLo ^ auchCRCHi[uIndex]);
			uchCRCLo = auchCRCLo[uIndex];
			i++;
		}
