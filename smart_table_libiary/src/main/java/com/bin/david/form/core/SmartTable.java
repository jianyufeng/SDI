 ...
 * 表格        release();//有风险的代码 365  在 DialogFragment 中
 */
    ...
    public void addData(final List<T> t, final boolean isFoot) {
        if (t != null && t.size() > 0) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    addDataSafe(t, isFoot);
                }
            }).start();
        }
    }

    private synchronized void addDataSafe(List<T> t, boolean isFoot) {
        isNotifying.set(true);
...
*/
        ...
        final int contentWidth = getWidth() - getPaddingLeft() - getPaddingRight();
        int scrollRange = matrixHelper.getZoomRect().right;
        final int scrollX = -matrixHelper.getZoomRect().left;//(修改1)
        final int overScrollRight = Math.max(0, scrollRange - contentWidth);
        if (scrollX < 0) {
            scrollRange -= scrollX;
        } else if (scrollX > overScrollRight) {
...
//        return Math.max(0, -matrixHelper.getZoomRect().top);
        return Math.max(0, -matrixHelper.getZoomRect().left);//(修改2)
    }
    ...
    public int computeVerticalScrollRange() {
      ...
      /*  final int contentHeight = getHeight() - getPaddingBottom() - getPaddingTop();
        ...
        final int contentHeight = getHeight() - getPaddingBottom() - getPaddingTop();
        int scrollRange = matrixHelper.getZoomRect().bottom;
        final int scrollY = -matrixHelper.getZoomRect().top;//(修改3)
        final int overScrollBottom = Math.max(0, scrollRange - contentHeight);
        if (scrollY < 0) {
            scrollRange -= scrollY;
        } else if (scrollY > overScrollBottom) {
...
//        return Math.max(0, -matrixHelper.getZoomRect().left);
        return Math.max(0, -matrixHelper.getZoomRect().top);//(修改4)
    }
