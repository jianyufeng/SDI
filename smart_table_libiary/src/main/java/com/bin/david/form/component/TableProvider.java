...
public class TableProvider<T> implements TableClickObserver {
    ...
    private PointF clickLongPoint;
    private ColumnInfo clickColumnInfo;
    private ColumnInfo clickLongColumnInfo;
    private boolean isClickPoint;
    private boolean isClickLongPoint;
    ...
    public void onDraw(Canvas canvas, Rect scaleRect, Rect showRect,
        ...
        } else if (isClickLongPoint && clickLongColumnInfo != null) {
            onColumnClickListener.onLongClick(clickLongColumnInfo);
        }
        if (tipColumn != null) {
        ...
        if (DrawUtils.isMixRect(showRect, left, top, right, bottom)) {
            if (!isClickPoint && onColumnClickListener != null) {
                if (DrawUtils.isClick(left, top, right, bottom, clickPoint)) {
                    ...
                    clickPoint.set(-1, -1);
                } else if (DrawUtils.isClick(left, top, right, bottom, clickLongPoint)) {
                    isClickLongPoint = true;
                    clickLongColumnInfo = info;
                    clickLongPoint.set(-1, -1);
                }
            }
    ...
    private void drawContent(Canvas canvas) {
        ...
        for (int i = 0; i < columns.size(); i++) {
            ...
            if (left < showRect.right) {
                int realPosition = 0;
                for (int j = 0; j < column.getDatas().size(); j++) {
